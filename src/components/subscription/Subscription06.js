...
function Subscription06({ info, date }) {
  ...
  const totalGeneral = detailList.reduce((sum, d) => sum + Number(d.suplyHshldco || 0), 0);
  const totalSpecial = detailList.reduce((sum, d) => sum + Number(d.spsplyHshldco || 0), 0);
    ...
    <>
      ...
      <div className='subscription_detail_section'>
              ...
              <table className='subscription_detail_basic_table'>
                <tbody>
                  ...
                  <tr>
                    <th>계약일</th>
                    <td colSpan={3} className='text-left'>
                      {fmt(info.cntrctCnclsBgnde)} ~ {fmt(info.cntrctCnclsEndde)}
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div className='subscription_detail_explan'>
          {specialList.length > 0 && <span>*특별사항 : {specialList.join(', ')}</span>}
          <span>
            *특별공급은 종류에 따라 접수기간 및 장소가 다를 수 있으니 모집공고를 반드시 확인하시길
            바랍니다.
          </span>
        </div>
      </div>
      ...
      <div className='subscription_detail_section'>
          ...
          <table className='subscription_detail_housing_table'>
            ...
            <tbody>
              {detailList.map((d, idx) => {
                const rowTotal = Number(d.suplyHshldco || 0) + Number(d.spsplyHshldco || 0);
                return (
                  <tr key={d.modelNo}>
                    {idx === 0 && (
                      <td rowSpan={detailList.length}>{info.houseDtlSecdNm || '민영'}</td>
                    )}
                    <td>{d.houseTy}</td>
        ...
        <div className='subscription_detail_explan'>
          <span>
            * 특별공급 신청 미달 시 잔여물량은 일반공급으로 전환됨에따라 일반공급 세대 수가 변경될
            수 있으므로 최종 일반공급 세대수는 일반공급 신청일에 &apos;청약접수 경쟁률&apos;에서
            확인 또는 사업주체에 문의하시기 바랍니다.
          </span>
      ...
      {/* ---------- 공급금액 및 입주예정월 ---------- */}
      <div className='subscription_detail_section'>
        <span className='subscription_detail_section_title'>공급금액 및 입주예정월</span>
        <div className='table_scroll'>
          <table className='subscription_detail_price_table'>
            ...
            <tbody>
              {detailList.map(d => (
                <tr key={d.modelNo}>
                  <td>{d.houseTy}</td>
                  <td className='border-right-1px'>{Number(d.lttotTopAmount).toLocaleString()}</td>
      ...
      <div className='subscription_detail_section'>
        ...
        <div className='subscription_detail_explan'>
          <span>*시행사 및 시공사가 여러 업체인 경우 한 업체만 표시됩니다.</span>
          {info.press ? (
            <span>
              *기타 자세한 모집공고문 내용은 {info.rcritPblancDe} {info.press}에 게시된 내용을
              참고하시기 바랍니다.
            </span>
          ) : (
            <span>
              *기타 자세한 모집공고문 내용은 사업주체 홈페이지 및 구청 게시판 등에 게시된 내용을
              참고하시기 바랍니다.
            </span>
          )}
        </div>
      </div>
      <span className='subscription_item_description'>&apos;청약&apos; 정보를 확인하세요</span>
    </>
  );
}
